...
שדרוג שמעביר את המתכנן מ"אורך רחוב + תדירות" בלבד לתכנון שמתחשב ברמת
הלכלוך בפועל, בזמן הניקיון האמיתי, בנקודות מים ופסולת, ובביצועים היסטוריים.
מסמך מלא: [docs/smart-cleaning-routing.md](docs/smart-cleaning-routing.md).

* **פרופיל ורצף למידה** (`/streets`, `/survey`) — כל רחוב מקבל `StreetCleaningProfile`
  עם ציון `DirtPriorityScore` שקוף (0–100), משוקלל ומוסבר תמיד לצד פירוק
  הגורמים. מתחיל מ-`REQUIRES_REVIEW`, עולה ל-`MANUAL_BASELINE` אחרי סקר,
  ל-`RULE_BASED` עם פרופיל מלא, ול-`DATA_INFORMED` רק אחרי 10 ביצועים
  מדווחים — הסולם עולה על סמך ראיות בלבד, לא הזנת טופס.
* **נקודות מים ופריקה** (`/service-points`) — הוספה בלחיצה על המפה, דיווח
  תקלה, שעות פעילות ומגבלות כלי. נקודה חדשה נוצרת `REQUIRES_REVIEW` עד לאימות.
* **פרופיל תפעולי לכלי** (`/resources`) — קיבולת מים/פסולת, צריכה, מידות
  והתאמת דרך. שדה לא ידוע מוצג ככזה, לעולם לא כברירת מחדל מומצאת.
* **מנוע מסלולים** (`/plans`) — מציע עד שלוש חלופות ("מהיר ביותר", "עדיפות
  ניקיון", "חסכוני במים ובנסיעות") עם פירוק עלות מלא לכל אחת, מוסיף עצירות
  מילוי/פריקה בדיוק כשנדרש (ולא נקודת מים תקולה), ובודק 11 קריטריוני
  היתכנות לפני פרסום — חריגה דורשת נימוק שנשמר ב-Audit Log.
* **דיווחי שדה ולמידה** (`/my-day`, `/learning`) — פתיחת משמרת וסיום עבודה
  קצרים ל-RTL/מגע; דיווח מזין ממוצע נע (EWMA) עם הגנה מפני דגימה חריגה
  (מסומנת "לא נכללה", לא נמחקת), ותיקון ידני נועל שדה מפני דריסה חוזרת.
* **ללא GPS/מצלמות** — לפי הוראה מפורשת, המערכת אינה מפתחת מעקב חי; תכנון
  מול ביצוע מבוסס אך ורק על דיווחי עובדים ומנהלי עבודה.
* **נתוני Demo**: `npm run seed:demo-routing` / `npm run clear:demo-routing`
  (או דרך `/settings`) — אזור הדגמה יחיד, מסומן `DEMO` בכל מקום, ואינו נכלל
  באף חישוב חיסכון או למידה.
...
npm run test:routing        # 52 בדיקות: ציון לכלוך, מים, נקודות שירות, היתכנות, למידה, +5 תרחישי §21
...
npm run smoke:routing       # end-to-end מול הדפדפן — סקר, נקודות שירות, חלופות מסלול, פרסום עם חריגה
npm run smoke:regression    # מעבר על כל המסכים
```

`test:spatial`, `test:defects`, `test:reports`, `test:allocation` ו-`test:routing` יוצרים נתוני בדיקה זמניים, בודקים מולם, ומנקים אחריהם — בסיס הנתונים נשאר כפי שהיה. `smoke:defects` דורש את משתמשי הבדיקה מ-`npm run seed:demo-roles`. `smoke:routing` מריץ בעצמו `seed:demo-routing`/`clear:demo-routing`.
